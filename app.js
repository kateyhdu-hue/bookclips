...
function moveSelection(e){if(!dragStart)return;e.preventDefault();const p=getPoint(e);selection={x:Math.min(dragStart.x,p.x),y:Math.min(dragStart.y,p.y),w:Math.abs(p.x-dragStart.x),h:Math.abs(p.y-dragStart.y)};updateSelectionBox()}
function endSelection(){if(!dragStart)return;dragStart=null;if(!selection||selection.w<20||selection.h<20){selection=null;hideSelection();$("ocrStatus").textContent="框选太小，请重新选择。";return}$("ocrStatus").textContent="已框选区域。点击识别。"}
...
function exportJson(){const blob=new Blob([JSON.stringify(state,null,2)],{type:"application/json"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`bookclips-${new Date().toISOString().slice(0,10)}.json`;a.click();URL.revokeObjectURL(url)}
